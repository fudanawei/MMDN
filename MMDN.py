# -*- coding: utf-8 -*-
"""
Created on Fri Oct 11 16:34:37 2022
@author: Wei Zhou, Ziwei Li
MMDN: Dynamic learning multi-memory neural network for image recovery through unstablized MMF
"""
import os
...
parser.add_argument("--datafolder", type=str, default="../Data/Testdata_bin32")
parser.add_argument("--saverootfolder", type=str, default="../Data/Testdata_bin32/result")
parser.add_argument("--use_model", type=str, default="pretrain", help="pretrain model prefix")
parser.add_argument("--modelfolder", type=str, default="../Data/Testdata_bin32/model")
...
parser.add_argument("--sizeOfUpdateInvertal", type=int, default=1000, help="update interval")
...
sizeOfBatch = args.sizeOfBatch        # number of dataset for each update 
sizeOfUpdateInvertal = args.sizeOfUpdateInvertal  # update interval
...
from model import createCNN, get_callbacks
...
# load target pattern
dataLabels = np.load(os.path.join(datafolder, 'pattern.npy')).astype(np.uint8)
outsize = dataLabels.shape[1]

# load input speckle
dataValues0 = np.load(os.path.join(datafolder, 'speckles.npy')).astype(np.uint8)
...
writepath_acc = os.path.join(saverootfolder, 'co_train_accuracy')
if not os.path.exists(writepath_acc):
    os.mkdir(writepath_acc)

writepath_diffimage = os.path.join(saverootfolder, 'co_train_diffimage')
if not os.path.exists(writepath_diffimage):
    os.mkdir(writepath_diffimage)

# record grayscale prediction and spatial difference
...
# init models
clf1 = createCNN(outsize=outsize, input_shape=input_shape)
clf1.compile(loss=keras.losses.binary_crossentropy, optimizer=ada)
clf2 = createCNN(outsize=outsize, input_shape=input_shape)
clf2.compile(loss=keras.losses.binary_crossentropy, optimizer=ada)
clf3 = createCNN(outsize=outsize, input_shape=input_shape)
...
for t in range(update_step+1, batches+update_step+1):
    ...
    finalDataLength = finalDataLength + sizeOfUpdateInvertal
    ...
    if pretrain_state and flag_dynamic_learning:
        ...
        if not os.path.exists(os.path.join(modelfolder, 'pretrain_clf3.hdf5')):
    ...
    x_labeled = np.concatenate((x_labeled, x_fortest), axis=0)[-sizeOfPretrain:] 
    y_labeled = np.concatenate((y_labeled, predsb), axis=0)
...
# save predictions
np.save(os.path.join(resultfolder, f'pre_enblem.npy'), np.array(y_labeled))
if flag_multi_model:
